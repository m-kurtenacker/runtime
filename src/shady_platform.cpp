...
void ShadyPlatform::launch_kernel(DeviceId dev, const LaunchParams &launch_params) {
    ...
    auto device = shady::get_device(shd_rt, dev);
    shady::Dispatch* d = shady::launch_kernel(program, device, launch_params.grid[0] / launch_params.block[0], launch_params.grid[1] / launch_params.block[1], launch_params.grid[2] / launch_params.block[2], launch_params.num_args, launch_params.args.data);
    assert(d);
